...
def requestImage(p, pn):
    ...
    # (2022 ls223qx) - Kudos to Albert - ah225ex
    # for finding a problem here and providing a fix!
    image = loadImage(p)
    if 'content-length' in r.headers:
        ...
        if not r.headers['content-length'].isdigit():
            ...
            print(f'Got instead: {contentLength}')
            return False
        elif int(r.headers['content-length']) != len(image):
            print(f'Error: {pn} content length incorrect')
            print(f'Image len is {len(image)} and the content length is {contentLength}.')
            return False
    else:
